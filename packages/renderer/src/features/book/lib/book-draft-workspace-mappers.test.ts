  ...
  return {
    ...
    readableManuscript: {
      ...
      markdown: [
        ...
        '',
        '> Transition gap: No artifact-backed transition draft has been materialized for this seam yet.',
        '',
        ...
        '### Scene 1: Dawn Slip',
        '',
        'Dawn slip prose now lands the revised harbor exit beat.',
      ].join('\n'),
      plainText: [
        'Signal Arc',
        '',
      ...
      sections: [
        ...
        },
        {
          ...
          toSceneId: 'scene-departure-bell',
          fromSceneTitle: { en: 'Midnight Platform', 'zh-CN': '午夜站台' },
          toSceneTitle: { en: 'Departure Bell', 'zh-CN': '离站钟声' },
          gapReason: {
            en: 'No artifact-backed transition draft has been materialized for this seam yet.',
            'zh-CN': '这条接缝还没有生成带产物引用的过渡草稿。',
          },
        },
        {
          kind: 'scene-gap',
          ...
          sceneId: 'scene-departure-bell',
          sceneOrder: 2,
          sceneTitle: { en: 'Departure Bell', 'zh-CN': '离站钟声' },
          sceneSummary: { en: 'Gap scene', 'zh-CN': '缺稿场景' },
          gapReason: {
            en: 'No prose artifact has been materialized for this scene yet.',
            'zh-CN': '该场景的正文产物尚未生成。',
          },
        ...
        },
        {
          ...
          chapterId: 'chapter-open-water-signals',
          chapterOrder: 2,
          chapterTitle: { en: 'Open Water Signals', 'zh-CN': '开阔水域信号' },
          summary: { en: 'Second chapter', 'zh-CN': '第二章' },
          assembledWordCount: 10,
          missingDraftCount: 0,
        },
        {
          ...
          sceneId: 'scene-dawn-slip',
          sceneOrder: 1,
          sceneTitle: { en: 'Dawn Slip', 'zh-CN': '黎明滑道' },
          sceneSummary: { en: 'Current prose', 'zh-CN': '当前正文' },
          proseDraft: 'Dawn slip prose now lands the revised harbor exit beat.',
          draftWordCount: 10,
          traceReady: true,
...
describe('buildBookDraftWorkspaceViewModel', () => {
  ...
  it('assembles accepted and revised scene prose into stable chapter sections while leaving missing prose as explicit gaps', () => {
    ...
    expect(workspace.readableManuscript.markdown).toContain('## Chapter 1:')
    expect(workspace.readableManuscript.sourceManifest.every((entry) => entry.kind === 'scene-draft' || entry.kind === 'scene-gap')).toBe(true)
    expect(workspace.readableManuscript.sourceManifest.some((entry) => entry.kind === 'transition-gap')).toBe(false)
  })
  ...
  it('maps a live draft assembly record into the existing book draft workspace shape while keeping gaps explicit', () => {
    ...
    expect(workspace.readableManuscript.formatVersion).toBe('book-manuscript-assembly-v1')
    expect(workspace.readableManuscript.markdown).toContain('## Chapter 1: Signals in Rain')
    expect(workspace.readableManuscript.plainText).toContain('Chapter 1: Signals in Rain')
    expect(workspace.readableManuscript.sections.map((section) => section.kind).slice(0, 4)).toEqual([
      'chapter-heading',
      'scene-draft',
      'transition-gap',
      'scene-gap',
    ])
    expect(workspace.readableManuscript.sourceManifest).toEqual(
