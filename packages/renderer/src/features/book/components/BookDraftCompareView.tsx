...
export function BookDraftCompareView({
  ...
  const selectedChapter = compare.selectedChapter
        ...
        <section className="rounded-md border border-line-soft bg-surface-1 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-lg text-text-main">{selectedChapter?.title ?? compare.title}</h3>
              {selectedChapter ? (
                <p className="mt-2 text-sm leading-6 text-text-muted">{selectedChapter.summary}</p>
              ) : null}
            </div>
            {selectedChapter ? (
              <Badge tone={getCompareChapterStatusBadge(getCompareChapterStatus(selectedChapter), locale).tone}>
                {getCompareChapterStatusBadge(getCompareChapterStatus(selectedChapter), locale).label}
              </Badge>
            ) : null}
          </div>
          {selectedChapter ? (
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                type="button"
                aria-label={`${locale === 'zh-CN' ? '在 Draft 中打开' : 'Open in Draft'}: ${selectedChapter.title}`}
                onClick={() => onOpenChapter(selectedChapter.chapterId, 'draft')}
                className="rounded-md border border-line-soft px-2 py-1 text-xs font-medium text-text-muted hover:bg-surface-2 hover:text-text-main focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
                {locale === 'zh-CN' ? '在 Draft 中打开' : 'Open in Draft'}
              </button>
              <button
                type="button"
                aria-label={`${locale === 'zh-CN' ? '在 Structure 中打开' : 'Open in Structure'}: ${selectedChapter.title}`}
                onClick={() => onOpenChapter(selectedChapter.chapterId, 'structure')}
                className="rounded-md border border-line-soft px-2 py-1 text-xs font-medium text-text-muted hover:bg-surface-2 hover:text-text-main focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Badge tone="neutral">{locale === 'zh-CN' ? `变更 ${selectedChapter?.totals.changedCount ?? 0}` : `Changed ${selectedChapter?.totals.changedCount ?? 0}`}</Badge>
            <Badge tone="neutral">{locale === 'zh-CN' ? `新增 ${selectedChapter?.totals.addedCount ?? 0}` : `Added ${selectedChapter?.totals.addedCount ?? 0}`}</Badge>
            <Badge tone="neutral">{locale === 'zh-CN' ? `缺失 ${selectedChapter?.totals.missingCount ?? 0}` : `Missing ${selectedChapter?.totals.missingCount ?? 0}`}</Badge>
            <Badge tone="neutral">
              {formatWordDelta(locale, selectedChapter?.wordDelta ?? 0)}
            </Badge>
            <Badge tone={(selectedChapter?.traceRegressionCount ?? 0) > 0 ? 'warn' : 'success'}>
              {locale === 'zh-CN'
                ? `溯源回退 ${selectedChapter?.traceRegressionCount ?? 0}`
                : `Trace regressions ${selectedChapter?.traceRegressionCount ?? 0}`}
            </Badge>
            <Badge tone={(selectedChapter?.warningsDelta ?? 0) > 0 ? 'warn' : 'neutral'}>
              {locale === 'zh-CN'
                ? `警告变化 ${(selectedChapter?.warningsDelta ?? 0) > 0 ? '+' : ''}${selectedChapter?.warningsDelta ?? 0}`
                : `Warnings ${(selectedChapter?.warningsDelta ?? 0) > 0 ? '+' : ''}${selectedChapter?.warningsDelta ?? 0}`}
            </Badge>
          </div>
