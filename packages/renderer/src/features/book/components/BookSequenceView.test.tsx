...
describe('BookSequenceView', () => {
  it('renders chapter cards with select and handoff actions', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Chapter 1 Signals in Rain' })).toHaveAttribute('aria-current', 'true')
    expect(screen.getAllByText('Missing trace 1')).toHaveLength(2)
    expect(screen.getByText('Carry platform pressure')).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /Chapter 2 Open Water Signals/i }))
