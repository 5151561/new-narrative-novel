...
export function BookBottomDock({ summary, activity }: BookBottomDockProps) {
  ...
  return (
    <section
      aria-label={locale === 'zh-CN' ? '书籍底部面板' : 'Book bottom dock'}
    ...
    >
      <PaneHeader
        ...
        description={
          locale === 'zh-CN'
            ? '底部面板只承接书籍级判断辅助和会话活动，不复制主舞台。'
            : 'The bottom dock keeps book-level judgment support and session-local activity without duplicating the stage.'
        }
      />
