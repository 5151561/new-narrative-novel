...
- `GET /api/projects/{projectId}/runs/{runId}/events/stream` 仍然保持 deferred；PR48 在本 bundle 内不引入 stream persistence，这部分继续留给 PR49。
...
PR39 不实现 SSE / WebSocket。当前 renderer 仍以 REST + polling/page contract 为准。
