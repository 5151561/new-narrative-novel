...
## 1.2 PR39 runtime mode boundary

当前前端 runtime mode 是刻意分层的：
...
- `GET /api/projects/{projectId}/runs/{runId}/events/stream` 提供 `text/event-stream` 传输层，复用现有产品级 `RunEventRecord`，不引入第二套事件 schema。

## PR54 Local Project Store v1

PR54 为“已选中的 desktop 项目”收紧了当前 project session 与本地持久化合同；对这一路径来说，它取代了旧的 workspace-root `prototype-state.json` 假设，改为以 project root 自带的 manifest + store 文件作为唯一事实来源。

- 已选中的 desktop 项目固定使用两份文件：
  - `<projectRoot>/narrative.project.json`
  - `<projectRoot>/.narrative/project-store.json`
- `narrative.project.json` 负责 current project metadata 与 store 定位；`.narrative/project-store.json` 负责该 project 的本地产品态数据。
- selected `projectId` 是当前 runtime/session identity，会同时出现在 `GET /api/current-project` 与 `GET /api/projects/{projectId}/runtime-info`。
- `bookId` / `chapterId` / `sceneId` / `assetId` 继续保持 route object identity；PR54 没有把这些对象 id 改写成 desktop session id，也没有把 route 语义并入 current-project metadata。
- 当 desktop 没有配置 current project 时，API 保持 fixture demo mode；这条路径不能写入 local project store，也不能偷偷初始化 `<projectRoot>/.narrative/project-store.json`。
- 若 `narrative.project.json` 或 `.narrative/project-store.json` 缺失，启动流程必须按 `signal-arc-demo-template-v1` 初始化所需文件与默认内容，再以当前 selected project identity 对外提供 `/api/current-project` 与 `/runtime-info`。
- 若本地 project store 文件存在但 malformed、schemaVersion 不兼容、或 kind 不兼容，启动必须直接失败；不允许静默回退到 fixture data 继续对外伪装成 healthy runtime。
- `ProjectRuntimeInfoRecord.versionLabel = local-project-store-v1` 表示当前 runtime-info 读取面正在消费这套本地 project store 合同。
...
- `draft-assembly` 的语义在 web/api 与 desktop-local API 路径上保持一致；mock fallback 只负责在 API 不可用时维持工作台读面可演示。
...
当前 API 同时保留两条运行事件读取路径：

- `GET /api/projects/{projectId}/runs/{runId}/events?cursor={eventId}`：继续提供分页 polling/fallback 合同，返回 `RunEventsPageRecord`。
- `GET /api/projects/{projectId}/runs/{runId}/events/stream?cursor={eventId}`：提供 `text/event-stream` SSE 传输，消息 `data` payload 仍是 `RunEventsPageRecord` 兼容结构。

SSE 路径的约束：

- `cursor` 为可选；提供后，stream 会先 replay 光标之后的现有产品事件，再 tail 后续 run-state 发布。
- stream payload 只承载既有 `RunEventRecord[]`；不会引入 provider transcript、worker history、prompt body 或第二套 event kind。
- `runEventStream` capability 只在该 SSE 路径实际可用时返回 `true`；`runEventPolling` 继续保留为 fallback 能力位。
- 当 runtime 明确未开放 stream transport 时，`GET /runs/{runId}/events/stream` 仍可返回 `501`，客户端应回退到分页 `/events` 合同。
- renderer 仍不 import Temporal SDK，也不直接消费 worker/provider 内部事件；产品 run events 仍是稳定的产品级 records。
...
- 它需要尽量保持 canonical fixture ids 与 API 路径一致；若存在 mock-only 或 API-only fixture 差异，应在审计文档中显式记录原因，而不是静默接受。
...
GET /api/projects/{projectId}/runs/{runId}/events/stream
  -> text/event-stream carrying RunEventsPageRecord-compatible payloads
```
...
Stream run events with cursor replay:

```http
GET /api/projects/project-run-contract/runs/run-scene-midnight-platform-002/events/stream?cursor=run-event-scene-midnight-platform-002-004
```
...
- `GET /runs/{runId}/events/stream` 现在是已落地的 SSE transport，但它仍然只是现有产品事件记录的另一种传输方式。
- paging contract 与 stream contract 共享同一套 `RunEventRecord` 语义；stream 不制造新的 event schema，也不暴露 engine/provider 原始 payload。
